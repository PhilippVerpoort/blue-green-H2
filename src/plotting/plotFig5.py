...
def plotFig5(fullParams: pd.DataFrame, fuelData: pd.DataFrame, scenario_name = "", export_img: bool = True):
...
def __getPlottingConfig():
    configThis = yaml.load(open('input/plotting/config_fig5.yml', 'r').read(), Loader=yaml.FullLoader)
    return configThis
