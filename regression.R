...
for(type in c('smr', 'heb', 'leb')) {
    ...
    if(type == 'smr') {
        model <- nls(LCOH ~ alpha * (epsilon*C_pl+fOnM_pl+vOnM_pl)/(P_pl*ohrs) + gamma * p_ng/eff, data=blueData, start=list(alpha=1.0, gamma=1.0))
        print(summary(model))
        print(modelr::rsquare(model, blueData))
    }
    else {
        model <- nls(LCOH ~ alpha * (epsilon*C_pl+fOnM_pl+vOnM_pl)/(P_pl*ohrs) + gamma * p_ng/eff + mu * c_CTS*emi, data=blueData, start=list(alpha=1.0, gamma=1.0, mu=1.0))
        print(summary(model))
        print(modelr::rsquare(model, blueData))
    }
}
